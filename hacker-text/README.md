...
```html
<script src="https://cdn.jsdelivr.net/gh/hoctienganhcungtruong/libraries@main/hacker-text/hacker-text-v5.min.js"></script>
...
| `messages` | `data-messages` | `Array` | `String` | `[InnerText <vertical-line> "HACKING..."]` &vert; Text configurations. Defaults to the element's inner text, or `"HACKING..."` if empty. Split via vertical line or pass a serialized JSON string array. |
...
| `interrupt` | `data-interrupt` | `Boolean` | `false` | Controls whether a user can break or restart an active animation by triggering it again (like hovering, clicking, or calling `.start()` manually) while it's still running.
